...
func getExpenseReport(e expense) (string, float64) {
	...
	switch itr := e.(type){
	case email:
	    return itr.toAddress , itr.cost()
	case sms:
	    return itr.toPhoneNumber , itr.cost()
	default:
	    return "",0.0
	}
	
/*	We can use this code block also
    This is basic type assertion
    (underlying type) em.method()
    em, ok := e.(email)
	if ok {
